...
When an external GPL scene renderer subprocess is bundled, desktop `scene.pkg` playback should prefer that renderer and fall back to the native renderer when the subprocess is unavailable, invalid, or fails to launch. The bundled macOS screen saver still uses the native/video support path described below. This project does not download Steam Workshop items, redistribute creator assets, or bundle Steam content; a renderer binary is a separate component with its own GPL source notice.

The external scene renderer also requires the original Wallpaper Engine runtime `assets` folder, copied by the user from their own Windows Wallpaper Engine installation. Workshop Wallpaper Bridge does not download or ship these files. Copy `steamapps/common/wallpaper_engine/assets` to `~/Library/Application Support/WorkshopWallpaperBridge/wallpaper-engine-assets`, or set `WWB_SCENE_ENGINE_ASSETS_DIR=/path/to/assets` before launching the app. If the renderer binary is present but these engine assets are missing or incomplete, the app keeps using the native scene fallback instead of launching a likely-black external render.
...
The package never bundles Wallpaper Engine runtime assets. For external scene rendering, copy your own Windows Wallpaper Engine `steamapps/common/wallpaper_engine/assets` folder to `~/Library/Application Support/WorkshopWallpaperBridge/wallpaper-engine-assets` or launch with `WWB_SCENE_ENGINE_ASSETS_DIR` pointing at that folder. `swift run wwbctl doctor` reports whether the renderer binary and required engine asset files are available.
