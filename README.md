...
When an external GPL scene renderer subprocess is bundled and `ffmpeg` is available, desktop `scene.pkg` playback renders that scene to a cached mp4 offscreen (the renderer never opens a visible window) and plays it back through the video-wallpaper path; it falls back to the native renderer while the cache is being built, or permanently if the subprocess, engine assets, or `ffmpeg` are unavailable. Once that cached video exists, the bundled macOS screen saver reuses it too (see Screen Saver below); until then, the screen saver shows the scene's still preview image. This project does not download Steam Workshop items, redistribute creator assets, or bundle Steam content; a renderer binary is a separate component with its own GPL source notice.
...
- Scene wallpapers, once they have a cached render-to-video (see Scene Wallpaper Support above): the screen saver reuses that same cached mp4. Switching wallpapers or displays updates the screen saver configuration immediately; a scene's first render still completes in the background, and the screen saver upgrades from the still preview to the cached video automatically as soon as that render finishes, without needing to play it again.
