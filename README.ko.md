...
[English](README.md) · [기여 안내](CONTRIBUTING.md) · [보안 정책](SECURITY.md) · [릴리즈](https://github.com/3x-haust/workshop-wallpaper-bridge/releases)
...
| `scene.pkg` 씬 월페이퍼 | 패키지 안의 2D image layer와 기본 keyframe 움직임 렌더링 |

scene 지원은 보수적입니다. 기본 image-layer scene은 동작하며, packed `.tex` texture, LZ4 block, 주요 DXT 형식, position/scale/rotation/opacity keyframe을 처리합니다. particle, audio-reactive script, custom shader, text layer, media integration, video/GIF texture animation은 생략되거나 Wallpaper Engine과 다르게 보일 수 있습니다.
