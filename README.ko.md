...
Mac이 잠겨 있는 동안 움직이게 하려면 **Animate Screen Saver**를 켜고 **Screen Saver Settings**를 누른 뒤 System Settings > Wallpaper > Screen Saver에서 **Workshop Wallpaper Bridge**를 선택합니다. 이 버튼은 System Settings를 열기 전에 번들된 화면 보호기를 먼저 설치합니다. macOS는 로그인/잠금화면 배경화면 자체를 라이브 영상으로 교체하는 공개 경로를 제공하지 않으므로, 이 앱은 화면 보호기 경로를 사용합니다. MP4, MOV, M4V 월페이퍼는 선택한 화면 보호기가 시작된 뒤 재생됩니다. 다른 형식은 정적 이미지 fallback을 사용합니다.
...
중요: macOS 설정에서 **Workshop Wallpaper Bridge**를 화면 보호기로 직접 선택해야 합니다. 앱은 번들된 화면 보호기를 설치하고 현재 선택한 월페이퍼 설정을 기록할 수 있지만, macOS가 선택한 화면 보호기를 강제로 바꾸지는 않습니다.
...
3. **Screen Saver Settings**를 누릅니다. 이 동작은 번들된 `.saver`를 `~/Library/Screen Savers`에 설치하거나 갱신하고 System Settings > Wallpaper를 엽니다.
